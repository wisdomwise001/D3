...
- All SofaScore traffic is routed through a rotating SOCKS5/HTTP proxy pool (`server/proxyFetch.ts`, list in `data/proxies.json`) to bypass Replit IP blocks. Sticky-good proxies are reused; failing ones are blacklisted with rotation up to 8 attempts.
